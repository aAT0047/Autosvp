...
def process_vcf(file_path, bam_dir, vcf_save_dir, bam_save_dir, encoding='latin1'):
    ...
    for idx, row in df.iterrows():
        ...
        clip_bam(row['Sample_ID'], chrom_start, max(0, pos_start - 10000000), pos_start + 1000000, bam_dir, output_bam_start)
        
        # 结束位置前1000000和后10000
        output_bam_end = os.path.join(bam_save_dir, f"{unique_id}_end.bam")
        clip_bam(row['Sample_ID'], chrom_end, max(0, pos_end - 1000000), pos_end + 1000000, bam_dir, output_bam_end)
...
# 使用示例
file_path = "/data/home/std_12/ShiHe/IG.csv"
bam_dir = "/data/home/std_12/ShiHe/IG_BAM"   # BAM文件的目录
vcf_save_dir = "/data/home/std_12/ShiHe/IGvcf"   # VCF文件保存目录
bam_save_dir = "/data/home/std_12/ShiHe/IGbam"   # BAM文件保存目录
