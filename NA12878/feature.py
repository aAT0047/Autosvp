...
# 定义要创建的文件夹路径
folder_path = "/home/cloudam/simulat_2/output_data"

# 使用os.makedirs()创建文件夹，如果文件夹已存在则不会重复创建
...
def process_and_save_to_csv(input_bam_file, output_csv_file, sample_id):
  ...
  #  print(f"Data for {sample_id} ({input_bam_file}) has been saved to {output_csv_file}")


# 定义BAM文件目录
bam_dir = "/home/cloudam/simulat_2/sample_bam/"
output_data_dir = "/home/cloudam/simulat_2/output_data/"
# 使用多核并行处理多个BAM文件
with concurrent.futures.ProcessPoolExecutor(max_workers=6) as executor:
    bam_files = [os.path.join(bam_dir, f"{i}.bam") for i in range(1, 4801)]
    csv_files = [os.path.join(output_data_dir, f"{i}_data.csv") for i in range(1, 4801)]
    sample_ids = [f"Sample_{i}" for i in range(1, 4801)]
    
    for _ in tqdm(executor.map(process_and_save_to_csv, bam_files, csv_files, sample_ids), total=len(bam_files)):
        pass
print('merged_data.csv')
# Specify the path
path = "/home/cloudam/simulat_2/output_data"

# Generate the list of filenames with the full path
files = [f"{path}/{i}_data.csv" for i in range(1, 4801)]
...
# From the second file onwards, read the content with headers but only append the data
for file in tqdm(files[1:], desc="Merging files", unit="file"):  # 使用tqdm包裹循环，以显示进度条
    if os.path.exists(file):
        df = pd.read_csv(file)
        merged_df = pd.concat([merged_df, df], ignore_index=True)

# Save the combined data to a new CSV file
merged_df.to_csv(f"{path}/merged_data.csv", index=False)

print("All CSV files merged into merged_data.csv")
